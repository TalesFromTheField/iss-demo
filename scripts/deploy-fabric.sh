...
info "Applying KQL schema via fab import..."

SCHEMA_DIR=$(mktemp -d)
SCHEMA_ITEM_DIR="$SCHEMA_DIR/iss-demo-kqldb.KQLDatabase"
mkdir -p "$SCHEMA_ITEM_DIR"
...
cat > "$SCHEMA_ITEM_DIR/.platform" <<PLATFORM
{
...
cat > "$SCHEMA_ITEM_DIR/DatabaseProperties.json" <<PROPS
{
    "databaseType": "ReadWrite",
    "parentEventhouseItemId": "$EVENTHOUSE_ID",
...
cat > "$SCHEMA_ITEM_DIR/DatabaseSchema.kql" <<KQL
// KQL script
// Auto-generated by deploy-fabric.sh — do not edit manually.
...
if fab import "$WORKSPACE_ID/$KQLDB_NAME.KQLDatabase" -i "$SCHEMA_ITEM_DIR" --force $VERBOSE 2>&1; then
    ok "KQL schema applied: ISS_Loc, Astronauts tables + streaming ingestion policy"
else
    warn "fab import reported an issue — re-run this script or apply kql/schema.kql manually."
fi
