...
## Project Knowledge
- **Tech Stack:** Azure Functions (Python v2), Azure Event Hubs, Microsoft Fabric (EventStreams, KQL Database), Power BI, Bicep, GitHub Actions
- **Languages:** Python 3.11
- **ADR Location:** `docs/decisions/`
- **Key Architectural Constraints:** Fabric resources (EventStreams, KQL DB, Power BI) cannot be deployed via Bicep — must use Fabric REST APIs or portal; Azure Functions on Consumption plan may have cold start delays for 5-second timer; Event Hub output bindings handle serialization (no manual base64 encoding)
- **Existing Patterns to Follow:** Python v2 decorator model for Functions; modular Bicep under `infra/modules/`; OIDC auth for CI/CD (no stored secrets); managed identity + RBAC for Function-to-Event Hub auth
