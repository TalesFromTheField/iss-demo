...
## Project Knowledge
- **CI/CD Platform:** GitHub Actions (OIDC auth via Entra, `dev` environment)
- **Cloud Provider:** Azure (Functions, Event Hubs) + Microsoft Fabric (EventStreams, KQL DB, Power BI)
- **IaC Tool:** Bicep (modular, under `infra/modules/`)
- **Container Runtime:** N/A (Azure Functions Consumption plan, no containers)
- **Orchestration:** N/A
- **Build Command:** `cd functions && pip install -r requirements.txt` (Python Functions)
- **Deploy Command:** `az deployment group create` (Bicep), `az functionapp deployment source config-zip` (Functions)
